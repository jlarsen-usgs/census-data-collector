    ...
    def get_timeseries(self, feature_name, sf3_variables=(),
        ...
        if self._censusobj is None:
            ...
            for year, url in TigerWebMapServer.base.items():
                ...
                if url == url0:
                    ...
                    tw = TigerWeb(self._shp, self._field,  # TODO: deleted self._radius - make sure this doesn't break anything
                                  self._filter)
        ...
        if isinstance(polygons, str):
            if polygons.lower() == "internal":
                polygons = self.get_albers_shape(feature_name)   # TODO: need to replace with gdf
                convert = False
